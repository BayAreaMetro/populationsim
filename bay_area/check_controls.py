...
if args.model_type == 'TM1':
    hh_taz_controls = os.path.join("households", "data", "{}_taz_summaries_{}.csv".format(args.run_num,args.model_year))
...
if args.model_type == 'TM1':
    ...
    for taz in hh_taz_controls.index:
        ...
        if hh_taz_controls['TOTHH'][taz] < 5:
...
# note: "total persons" represents the hh_size_4_plus category as 4 persons, so in some cases it may underestimate 
# in those cases, tot_pop for the TAZ can be checked for a better estimate (through MAZ aggregation)

if args.model_type == 'TM1':
    for taz in hh_taz_controls.index:
        taz_total_hh_persons = (hh_taz_controls['hh_size_1'][taz])*1 + (hh_taz_controls['hh_size_2'][taz])*2 + (hh_taz_controls['hh_size_3'][taz])*3 + (hh_taz_controls['hh_size_4_plus'][taz])*4
...
# 3) for each TAZ, check that the number of hhs with/without kids makes sense given the number of persons of particular age groups

# household kids is not used as a control for TM1
