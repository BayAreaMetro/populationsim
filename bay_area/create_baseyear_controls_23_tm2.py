...
class CensusFetcher:
    ...
    CENSUS_DEFINITIONS = {
        "H13":[  # sf1, H13. Household Size [8]
            ...
            ["H013001",         1,   NPER_MAX], # Occupied housing units
            ["H013002",         1,          1], #  1-person household
            ["H013003",         2,          2], #  2-person household
            ["H013004",         3,          3], #  3-person household
            ["H013005",         4,          4], #  4-person household
            ["H013006",         5,          5], #  5-person household
            ["H013007",         6,          6], #  6-person household
            ["H013008",         7,   NPER_MAX], #  7-or-more-person household
        ],
    ...
    def get_census_data(self, dataset, year, table, geo):
        ...
        if os.path.exists(table_cache_file):
            ...
            # --------------------------
            # STEP 1: Grab the MultiIndex column names
            # --------------------------
            full_df_cols = pandas.read_csv(
                table_cache_file,
                header=list(range(len(table_cols))),  # MUST be a list of ints
                nrows=0
            ).columns

            # --------------------------
            # STEP 2: Read all data rows as plain data (header=None)
            # --------------------------
            dtypes_dict = {col: object for col in geo_index}
            raw_df = pandas.read_csv(
                table_cache_file,
                dtype=dtypes_dict,
                skiprows=len(table_cols),  # skip exactly the number of header-rows
                header=None               # do not treat any row as a header
            )

            # --------------------------
            # STEP 3: Split into "geo part" vs "data part"
            # --------------------------
            geo_part  = raw_df.iloc[:, : len(geo_index)].astype(str)
            data_part = raw_df.iloc[:, len(geo_index) :]

            # Sanity check: number of data columns must match number of full_df_cols
            if data_part.shape[1] != len(full_df_cols):
                raise ValueError(
                    f"Cached CSV has {data_part.shape[1]} data columns, "
                    f"but expected {len(full_df_cols)} (from header)."
                )

            # Assign the MultiIndex column names onto data_part
            data_part.columns = full_df_cols

            # Build a MultiIndex from the geo_part DataFrame
