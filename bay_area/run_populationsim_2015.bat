...
set PYTHONPATH=%USERPROFILE%\Documents\activitysim;%USERPROFILE%\Documents\populationsim
...
:create_seed
rem python create_seed_population.py
...
for %%Y in (2015) do (
  ...
  rem create controls
  rem python create_baseyear_controls.py !TEST_PUMA_FLAG! %%Y
  if ERRORLEVEL 1 goto error
  ...
  rem group_quarters
  rem mkdir group_quarters\output_%%Y!PUMA_SUFFIX!
  rem python run_populationsim.py --model_year %%Y --config group_quarters\configs --output group_quarters\output_%%Y!PUMA_SUFFIX!  --data group_quarters\data
