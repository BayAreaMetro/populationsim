...
class PopulationSimWorkflow:
    ...
    def _validate_grouping_columns(self):
        ...
        self.log("🔧 TARGETED GROUPING COLUMN VALIDATION...")
        ...
        if not hh_file.exists():
            self.log("  🚨 ERROR: Household file not found for grouping validation!", "ERROR")
            return False
        
        try:
            # Read household data
            df_hh = pd.read_csv(hh_file)
            self.log(f"  📊 Validating {len(critical_grouping_cols)} critical grouping columns in {df_hh.shape[0]} households")
            ...
            for col in critical_grouping_cols:
                ...
                if original_nan > 0 or original_inf > 0:
                    self.log(f"    🚨 CRITICAL FIX NEEDED for {col}: {original_nan} NaN, {original_inf} inf values")
                    ...
                    if final_nan > 0 or final_inf > 0:
                        ...
                        self.log(f"    ✅ FIXED {col}: Now clean, dtype={df_hh[col].dtype}")
                else:
                    self.log(f"    ✅ {col}: Clean ({df_hh[col].dtype})")
            
            # Save the fixed file if any fixes were applied
            if fixes_applied:
                self.log(f"  💾 SAVING FIXED DATA: Applied {len(fixes_applied)} critical fixes")
                ...
                if not backup_file.exists():
                    ...
                    self.log(f"    💾 Created backup: {backup_file.name}")
                
                # Save the fixed data
                df_hh.to_csv(hh_file, index=False)
                self.log(f"    💾 Saved fixed households file with clean grouping columns")
                ...
                for col in critical_grouping_cols:
                    if col in df_verify.columns:
                        ...
                        if verify_nan > 0 or verify_inf > 0:
                            self.log(f"    🚨 VERIFICATION FAILED for {col}: {verify_nan} NaN, {verify_inf} inf", "ERROR")
                        else:
                            self.log(f"    ✅ VERIFIED {col}: Clean after save")
            
            # Final validation summary
            if issues_found:
                self.log(f"  🚨 REMAINING ISSUES: {len(issues_found)} problems found:", "ERROR")
                for issue in issues_found:
                    self.log(f"    ❌ {issue}", "ERROR")
                return False
            else:
                self.log(f"  ✅ GROUPING VALIDATION COMPLETE: All {len(critical_grouping_cols)} critical columns are clean")
                return True
                
        except Exception as e:
            self.log(f"  🚨 ERROR during grouping validation: {e}", "ERROR")
            import traceback
            self.log(f"  📋 Validation traceback: {traceback.format_exc()}", "ERROR")
            return False
        ...
        try:
            ...
            try:
                ...
                if not validation_success:
                    self.log("🚨 CRITICAL: Grouping column validation failed! This will cause IntCastingNaNError.", "ERROR")
                ...
                while True:
                    ...
                    if output:
                        ...
                        if "populationsim.steps." in output:
                            print(f"📍 POPULATIONSIM STEP: {output_stripped}")
                        ...
                        if "control target" in output:
                            print(f"🎯 CONTROL TARGET: {output_stripped}")
