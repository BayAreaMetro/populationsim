...
set TMPATH=M:\Development\PopulationSynthesizer\TM2_inputs_outputs_PopulationSim\inputs\2015
...
set BAUS_RUNNUM=base
:: OUTPUT DIR will be X:\populationsim_outputs\hh_gq\output
set OUTPUT_SUFFIX=census
...
for %%Y in (!YEARS!) do (
  ...
  if !MODELTYPE!==TM2 (
    if !YEAR!==2015 (
      copy /y "%TMPATH%\maz_marginals.csv"    hh_gq\data\maz_marginals_2015.csv
      copy /y "%TMPATH%\taz2_marginals.csv"   hh_gq\data\taz_marginals_2015.csv
      copy /y "%TMPATH%\county_marginals.csv" hh_gq\data\county_marginals_2015.csv

      rem Verify that the file copy MUST succeed or we'll run populationsim with the wrong input
      fc /b "%TMPATH%\maz_marginals.csv"    hh_gq\data\maz_marginals_2015.csv > nul
      if errorlevel 1 goto error
      fc /b "%TMPATH%\taz2_marginals.csv"   hh_gq\data\taz_marginals_2015.csv > nul
      if errorlevel 1 goto error
      fc /b "%TMPATH%\county_marginals.csv" hh_gq\data\county_marginals_2015.csv > nul
      if errorlevel 1 goto error
    )
  )
  ...
  set OUTPUT_DIR=M:\Development\PopulationSynthesizer\TM2_inputs_outputs_PopulationSim\output
