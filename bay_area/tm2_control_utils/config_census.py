...
INCOME_BIN_MAPPING = [
    ...
    },
    {
        ...
        '2010_bin': (14068, 31653),
    },
    {
        'control': 'inc_45k_60k',
        'acs_vars': ['B19001_010E', 'B19001_011E'],
        '2023_bin': (45000, 59999),
        '2010_bin': (31653, 42204),
    },
    {
        'control': 'inc_60k_75k',
        'acs_vars': ['B19001_012E'],
        '2023_bin': (60000, 74999),
        '2010_bin': (42204, 52755),
    },
    {
        'control': 'inc_75k_100k',
        'acs_vars': ['B19001_013E'],
        '2023_bin': (75000, 99999),
        '2010_bin': (52755, 70340),
    },
    {
        'control': 'inc_100k_150k',
        'acs_vars': ['B19001_014E', 'B19001_015E'],
        '2023_bin': (100000, 149999),
        '2010_bin': (70341, 105510),
    },
    {
        'control': 'inc_150k_200k',
        'acs_vars': ['B19001_016E'],
        '2023_bin': (150000, 199999),
        '2010_bin': (105511, 140681),
    },
    {
        'control': 'inc_200k_plus',
        'acs_vars': ['B19001_017E'],
        '2023_bin': (200000, 9999999),
        '2010_bin': (140681, 7034064),
    },
]
#!/usr/bin/env python3
"""
...
NETWORK_MAZ_TAZ_DEF_FILE = r"C:\GitHub\tm2py-utils\tm2py_utils\inputs\maz_taz\blocks_mazs_tazs_2.6.csv"
NETWORK_MAZ_TAZ_ALL_GEOG_FILE = "geo_cross_walk_tm2_block10.csv"
NETWORK_CENSUS_API_KEY_FILE = r"M:\Data\Census\API\new_key\api-key.txt"



# Geographic data files - Using verified 2010 Census blocks
BLOCKS_MAZ_TAZ_FILE = r"C:\GitHub\tm2py-utils\tm2py_utils\inputs\maz_taz\blocks_mazs_tazs_2.6.csv"  # Updated to 2.6: expanded MAZ coverage (41,435 MAZs)

# Output file names
GEO_CROSSWALK_TM2_FILE = "geo_cross_walk_tm2_maz.csv"
MAZ_TAZ_ALL_GEOG_OUTPUT_FILE = "geo_cross_walk_tm2_block10.csv"  # Rebuilt complete geography file
COUNTY_TARGETS_FILE = "county_targets_2023.csv"
MAZ_MARGINALS_FILE = "maz_marginals_hhgq.csv"
...
def enforce_hierarchical_consistency(maz_controls, taz_controls, crosswalk_df=None):
    ...
    # Detect column naming convention (MAZ/TAZ vs MAZ_NODE/TAZ_NODE)
    ...
    print(f"Using column names: MAZ={maz_col}, TAZ={taz_col}")
    print(f"Crosswalk column names: MAZ={crosswalk_maz_col}, TAZ={crosswalk_taz_col}")
    ...
    maz_with_taz = maz_controls.merge(crosswalk_df[[crosswalk_maz_col, crosswalk_taz_col]], 
                                       left_on=maz_col, right_on=crosswalk_maz_col, how='left')
    
    # Check if we have num_hh and total_pop columns
    agg_cols = {}
    if 'num_hh' in maz_with_taz.columns:
        agg_cols['num_hh'] = 'sum'
    if 'numhh' in maz_with_taz.columns:
        agg_cols['numhh'] = 'sum'
    if 'total_pop' in maz_with_taz.columns:
        agg_cols['total_pop'] = 'sum'
    
    if not agg_cols:
        print("WARNING: No num_hh/numhh or total_pop columns found for aggregation")
        return maz_updated, taz_updated
    
    # Group by TAZ and sum MAZ totals
    taz_targets = maz_with_taz.groupby(crosswalk_taz_col).agg(agg_cols).reset_index()
    # Rename to standard TAZ column for internal consistency
    taz_targets = taz_targets.rename(columns={crosswalk_taz_col: taz_col})
    ...
    for maz_control, taz_control_list in HIERARCHICAL_CONSISTENCY.items():
        ...
        taz_with_targets = taz_updated.merge(taz_targets[[taz_col, maz_control]], on=taz_col, how='left')
